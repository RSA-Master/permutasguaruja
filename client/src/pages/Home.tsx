...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="practical-example section-light" id="exemplo-pratico"><div className="container"><div className="section-heading"><div><p className="eyebrow"><span /> Exemplo prático</p><h2>Imagine esta <em>situação.</em></h2></div><p>Uma história ilustrativa para entender como uma permuta pode resolver uma necessidade real sem comprometer o caixa.</p></div><div className="trade-carousel reveal-stagger" aria-label="Jornada ilustrativa de uma permuta entre uma loja e um profissional"><div className="trade-track trade-track--five" style={{ transform: `translateX(-${tradeSlide * 100}%)` }}><article className="trade-card trade-card--store"><span className="trade-index">01 / 05</span><Store size={23} /><p className="card-kicker">Você tem uma loja</p><h3>Loja de roupas</h3><strong>Sua loja precisa de melhorias para ficar ainda melhor.</strong></article><article className="trade-card trade-card--need"><span className="trade-index">02 / 05</span><Wrench size={23} /><p className="card-kicker">Você precisa</p><h3>Rebaixar o teto</h3><strong>Um serviço que pode melhorar o espaço e a experiência dos clientes.</strong></article><article className="trade-card trade-card--search"><span className="trade-index">03 / 05</span><UsersRound size={23} /><p className="card-kicker">Você procura</p><h3>Um profissional</h3><strong>Alguém do grupo disposto a realizar o serviço em troca de produtos.</strong></article><article className="trade-card trade-card--exchange"><span className="trade-index">04 / 05</span><Handshake size={23} /><p className="card-kicker">A negociação acontece</p><h3>Permuta</h3><strong>Roupas e produtos no valor combinado pelo serviço realizado.</strong></article><article className="trade-card trade-card--win"><span className="trade-index">05 / 05</span><CircleDollarSign size={23} /><p className="card-kicker">Todos ganham</p><h3>Valor em movimento</h3><strong>A loja resolve sua melhoria. O profissional ocupa um horário e pode gerar novos contatos.</strong></article></div><div className="trade-carousel-controls"><button type="button" aria-label="Etapa anterior" onClick={() => setTradeSlide((slide) => Math.max(0, slide - 1))} disabled={tradeSlide === 0}><ArrowLeft size={17} /></button><div className="trade-dots" aria-label={`Etapa ${tradeSlide + 1} de 5`}>{[0,1,2,3,4].map((dot) => <button type="button" key={dot} aria-label={`Ir para etapa ${dot + 1}`} aria-current={tradeSlide === dot} onClick={() => setTradeSlide(dot)} />)}</div><button type="button" aria-label="Próxima etapa" onClick={() => setTradeSlide((slide) => Math.min(4, slide + 1))} disabled={tradeSlide === 4}><ArrowRight size={17} /></button></div></div><p className="trade-note">A negociação depende do interesse e das condições acordadas entre as partes. <strong>Cada permuta é combinada diretamente pelos participantes.</strong></p></div></section>
