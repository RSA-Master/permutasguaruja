...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="practical-example section-light" id="exemplo-pratico"><div className="container"><div className="section-heading"><div><p className="eyebrow"><span /> Exemplo prático</p><h2>Imagine esta <em>situação.</em></h2></div><p>Uma forma simples de visualizar como uma permuta pode aproximar necessidades diferentes.</p></div><div className="trade-carousel reveal-stagger" aria-label="Etapas ilustrativas de uma permuta"><div className="trade-track" style={{ transform: `translateX(-${tradeSlide * 100}%)` }}><article className="trade-card trade-card--store"><span className="trade-index">01</span><Store size={23} /><p className="card-kicker">Loja de roupas</p><h3>Oferece</h3><strong>Roupas e produtos da loja</strong></article><article className="trade-card trade-card--exchange"><span className="trade-index">02</span><Handshake size={23} /><p className="card-kicker">Permuta</p><h3>Conecta</h3><strong>Valor combinado entre as partes</strong></article><article className="trade-card trade-card--professional"><span className="trade-index">03</span><Wrench size={23} /><p className="card-kicker">Profissional</p><h3>Recebe</h3><strong>Rebaixamento de teto + reforma da fachada</strong></article></div><div className="trade-carousel-controls"><button type="button" aria-label="Etapa anterior" onClick={() => setTradeSlide((slide) => Math.max(0, slide - 1))} disabled={tradeSlide === 0}><ArrowLeft size={17} /></button><div className="trade-dots" aria-label={`Etapa ${tradeSlide + 1} de 3`}>{[0,1,2].map((dot) => <button type="button" key={dot} aria-label={`Ir para etapa ${dot + 1}`} aria-current={tradeSlide === dot} onClick={() => setTradeSlide(dot)} />)}</div><button type="button" aria-label="Próxima etapa" onClick={() => setTradeSlide((slide) => Math.min(2, slide + 1))} disabled={tradeSlide === 2}><ArrowRight size={17} /></button></div></div><p className="trade-note">A loja fornece produtos no valor negociado e recebe os serviços necessários. <strong>Todos saem ganhando.</strong> Os exemplos são ilustrativos; cada permuta é negociada diretamente entre os participantes.</p></div></section>
